# -*- coding: utf-8 -*-
"""
Created on Sun Sep 15 18:53:34 2019
...
for pair in pairs:
    ...
    if USE_ALL_COLS:
        d = d.resample('1H').ohlc()
    else:
        d = d.resample('1H').agg({'Open_'+pair_cap: 'first', 
                                         'High_'+pair_cap: 'max', 
                                         'Low_'+pair_cap: 'min', 
    ...
    #-- Filter to only STARTHOUR snapshots
    d['Time']  = pd.to_datetime(d.index)
    d = d.loc[d.index.hour==START_HOUR,:] 
    ...
    print('Saving csv')
    #-- Write to csv
    rs.to_csv(data_output_dir + pair+'_tsfresh_feats_STARTHOUR_'+str(START_HOUR)+'_daily_returns.csv')
